...
- **`{{FORK}}{{ADAPTER}} --verify` 真库体检**：发布/环境变化后必跑；打分支前建议跑。输出分三档，**别误读**：
...
<!--WBS:-->5. **WorkBuddy 会追加消息到分支文件**（**这条只对 WorkBuddy 成立，不要套到其他产品**）：脚本创建分支后，WorkBuddy 主进程可能仍向该 jsonl 追加新消息。v1.4.0 起不再自动锁只读（执行后提示用户自行设为只读），已有分支可用 `--fix` 修复（**仅 workbuddy**）。<!--:WBS-->
