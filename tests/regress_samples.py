#!/usr/bin/env python3
"""session-fork 基线回归 —— 用**真实会话样本**钉住行为，防"悄悄退化"。
...
用法：
  python3 tests/regress_samples.py --baseline-out /tmp/samples.json   # 采基线
  python3 tests/regress_samples.py --baseline-in  /tmp/samples.json   # 回归（漂移即非零退出）
"""
...
#: ⚠️ **样本 id 不进仓库**：真实会话 id 属用户数据，写进本文件会随公开仓库一起发出去。
#: 因此样本清单来自**本地配置**（默认 `tests/samples.local.json`，已 gitignore；
#: 可用环境变量 `SAMPLES_CONFIG` 覆盖）。仓库里只保留这个脚本与格式约定。
SAMPLES_CONFIG = os.environ.get("SAMPLES_CONFIG") or os.path.join(
    REPO, "tests", "samples.local.json")

#: 配置格式：{"samples": [{"id": "...", "why": "...", "stable": true|false}, ...]}
...
def load_samples() -> list[dict]:
    global SAMPLES
    if not os.path.exists(SAMPLES_CONFIG):
        return []
    with open(SAMPLES_CONFIG, encoding="utf-8") as f:
...
def run_one(sid: str, adapter: str = "workbuddy", timeout: int = 600) -> dict:
    """对单个样本跑一次 dry-run，抽出可比对的事实。"""
    cmd = [PY, "-m", "fork_core.cli", "--adapter", adapter,
           "--session", sid, "--dry-run", "--name", "基线回归"]
...
def main() -> int:
    ...
    if not load_samples():
        print(f"（跳过）未找到本地样本配置：{SAMPLES_CONFIG}")
        print("        把 tests/samples.local.json.example 复制成 tests/samples.local.json、")
    ...
    if a.baseline_in:
        ...
        if errs:
            ...
            for e in errs:
                print("   " + e)
            return 1
        print(f"\n✅ 基线回归通过：{len(cur)} 条全部与基线一致（不变量 + 稳定样本的精确值）")
    return 0
